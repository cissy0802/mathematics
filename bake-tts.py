...
def _in_diagram(s) -> bool:
    ...
    _DIAGRAM_CLASSES = {"mermaid","diagram","viz","chart","graphviz","plot",
                        "formula","formula-box","equation","eq","math","latex","katex","mathjax"}
    _DIAGRAM_SUBSTR = ("diagram","formula","mermaid","mathjax","katex")
    def _isdia(cl):
        toks = set(cl if isinstance(cl, list) else [cl])
        if toks & _DIAGRAM_CLASSES: return True
        j = " ".join(toks)
        return any(sub in j for sub in _DIAGRAM_SUBSTR)
    return s.find_parent(class_=lambda cl: bool(cl) and _isdia(cl)) is not None
...
def normalize_for_tts(text: str) -> str:
    ...
    import re as _re0
    text = _re0.sub(r"\$\$.+?\$\$", "", text, flags=_re0.S)
    text = _re0.sub(r"\\\[.+?\\\]", "", text, flags=_re0.S)
    text = _re0.sub(r"\\\(.+?\\\)", "", text)
    def _inline_math(mm):
    ...
    # Removing a formula can leave dangling punctuation ("套娃：。") or double
    # spaces; tidy so the sentence reads smoothly.
    text = _re0.sub(r"[：:，,]\s*(?=[。；;！？])", "", text)
    text = _re0.sub(r"\s{2,}", " ", text)
    text = _re0.sub(r"(（\s*）|\(\s*\))", "", text)
    text = text.replace("❌ ", "反例，").replace("❌ ", "反例，").replace("❌", "反例，")
